...
app = typer.Typer(help="Authorship attribution and interpretability toolkit for Ancient Greek texts.")
...
@app.command()
def ingest(
    input_paths: Annotated[list[Path], typer.Option("--input", exists=True, help="Author directory to ingest (can be repeated).")],
    output: Annotated[Path, typer.Option(help="Output JSONL file path.")],
    source: Annotated[Source, typer.Option(help="Corpus format to ingest.")] = Source.first1kgreek,
    strip_section_numbers: Annotated[bool, typer.Option(help="Remove section numbers from passage text.")] = False,
) -> None:
    ...
    if source == Source.first1kgreek:
        ...
        count = do_ingest(input_paths, output, strip_section_numbers)
    else:
        typer.echo(f"Unknown source format: {source}", err=True)
        raise typer.Exit(1)
...
@app.command()
def chunk(
    input_path: Annotated[Path, typer.Option("--input", exists=True, help="Input corpus JSONL file.")],
    ...
    overlap: Annotated[int, typer.Option(help="Token overlap between chunks. 0 for training, >0 for sliding window.")] = 0,
) -> None:
...
def train(
    input_path: Annotated[Path, typer.Option("--input", exists=True, help="Input corpus JSONL file.")],
    output_dir: Annotated[Path, typer.Option(help="Output directory for model and predictions.")],
    positive_author: Annotated[str, typer.Option(help="Author ID for the positive class (e.g. tlg0057).")],
    model_name: Annotated[str, typer.Option(help="HuggingFace model name or path.")] = "pranaydeeps/Ancient-Greek-BERT",
    ...
    model_dir = do_train(
        input_path, output_dir, positive_author,
...
def predict(
    ...
    count = do_predict(input_path, model_dir, output, positive_author, max_length, batch_size)
    typer.echo(f"Wrote {count} predictions to {output}")


@app.command()
def evaluate(
    input_path: Annotated[Path, typer.Option("--input", exists=True, help="Predictions JSONL file.")],
...
@app.command()
def explain(
    input_path: Annotated[Path, typer.Option("--input", exists=True, help="Predictions JSONL file to explain.")],
    model_dir: Annotated[Path, typer.Option(exists=True, help="Directory containing the saved model.")],
    output: Annotated[Path, typer.Option(help="Output explanations JSONL file.")],
    num_features: Annotated[int, typer.Option(help="Number of top features per explanation.")] = 10,
    num_samples: Annotated[int, typer.Option(help="Number of perturbed samples for LIME.")] = 5000,
    html_dir: Annotated[Path | None, typer.Option(help="Directory to save HTML visualizations.")] = None,
    ...
    count = do_explain(input_path, model_dir, output, num_features, num_samples,
                       html_dir=html_dir, seed=seed)
...
def export_activations(
    input_path: Annotated[Path, typer.Option("--input", exists=True, help="Input corpus JSONL file.")],
    model_dir: Annotated[Path, typer.Option(exists=True, help="Directory containing the saved model.")],
    output_dir: Annotated[Path, typer.Option(help="Output directory for .npy files and manifest.")],
    layers: Annotated[list[int] | None, typer.Option(help="Layer indices to export (can be repeated). Defaults to last layer.")] = None,
    include_token_embeddings: Annotated[bool, typer.Option(help="Also export per-token embeddings.")] = False,
    max_length: Annotated[int, typer.Option(help="Maximum token length.")] = 512,
    ...
    count = do_export(input_path, model_dir, output_dir, layers,
                      include_token_embeddings, max_length, batch_size)
